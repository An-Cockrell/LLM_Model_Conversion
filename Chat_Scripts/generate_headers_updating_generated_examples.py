...
def generate_headers_updating(
    ...
    # IF output from a previous run has been verified and is good, load that
    previous_output_files = ["LLM_Model_Conversion/LLM_output_files/headers_generated_deepseek8_2_manually_verified.json"]
    valid_generated_headers = []
    valid_generated_headers_str = ""
    netlogo_phrases = []
    valid_generated_headers_nlogo_as_key = {}
    for file_to_chunk in previous_output_files:
        with open (file_to_chunk, "r") as f:
            codeblocks = json.load(f)
            for key, value in codeblocks.items():
                valid_generated_headers.append(value["extracted_code"])
                valid_generated_headers_str += value["extracted_code"] + "\n\n"
                netlogo_phrases.append(value["netlogo_input"])
                valid_generated_headers_nlogo_as_key[value["netlogo_input"]] = value["extracted_code"]
    ...
    netlogo_phrases_for_embedding = []
    for phrase in netlogo_phrases:
        netlogo_phrases_for_embedding.append(["Represent block of netlogo:", phrase])
    netlogo_phrase_embeddings = embedding_model.encode(netlogo_phrases_for_embedding)
    ...
    system_prompt = "You are a helpful assistant who is an expert programmer. You will receive a command which you will help complete. Any code you write should be enclosed in triple backticks, like this ```// insert your code here```."

    prompt = "I am going to give you a notes file from a C++ simulation that I wrote. I will then give you some examples of what a good netlogo to C++ translation look like. Finally, I will give you a piece of netlogo that I want you to translate into a C++ header file."
    ...
    for block_id in blocks_to_label.keys():
        ...
        while produced_code == "" and num_tries < max_tries:
            ...
            block_message_content += "\nHere is the cursory explanation of the specific implementation:\n#IMPLEMENTATION EXPLANATION\n" + str(notes_text) + "\n#END IMPLEMENTATION EXPLANATION\n"


            # DO SOME RAG TO TRY TO GET SOME GOOD RELEVANT HEADER EXAMPLES
            MAX_NUM_EXAMPLES_TO_GIVE = 5
            # get embedding (vector representation of a string)
            block_embedding = embedding_model.encode([["Represent the netlogo phrase:", block]])
            # use cosine similarity to find other similar strings
            relevancy_to_other_netlogo = cosine_similarity(block_embedding, netlogo_phrase_embeddings).flatten().argsort()[::-1] #sort and then flip, so most relevant is first
            count = 0
            for i in relevancy_to_other_netlogo:
                netlogo_phrase = netlogo_phrases[i]
                example_code = valid_generated_headers_nlogo_as_key[netlogo_phrase]
                if example_code == "":
                    continue
                if count == 0:
                    block_message_content += "\nHere are examples of what I expect the translation output to look something like:\n#BEGIN TRANSLATION EXAMPLES\n"

                block_message_content += "\n;;NETLOGO\n" + netlogo_phrase
                block_message_content += "\n\n//CPP\n" + example_code + "\n\n\n"
                count += 1
                if count >= MAX_NUM_EXAMPLES_TO_GIVE:
                    break
            
            if count > 0:
                block_message_content += "#END TRANSLATION EXAMPLES\n"


            # getting one block of netlogo code from context dict
            block_message_content += "\nHere is the block of netlogo code I would like you to translate:\n#NETLOGO CODE\n" + str(block)

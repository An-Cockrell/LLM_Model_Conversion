...
{
   // Previous initialization code goes here

   for (int x = 0; x < WORLD_WIDTH; ++x) {
       for (int y = 0; y < WORLD_HEIGHT; ++y) {
           add_patch(x, y);
           Patch& currentPatch = all_patches[x][y]; // Access the current patch
           currentPatch.set_grass(100); // Initialize the grass amount
       }
   }

   // Additional code for updating grass goes here, if needed
...
// ...existing World class implementation...

void World::add_deer(int x, int y, int id, int heading) {
   auto deer = std::make_shared<Deer>(x, y, id, heading);
   Patch& patch = this->get_patch(x, y); // Assumes a 'get_patch' function is defined
   std::weak_ptr<Turtle> new_deer = deer;
   patch.add_turtle(new_deer);
   this->all_deers.push_back(deer);
}

void World::add_wolf(int x, int y, int id, int heading) {
   auto wolf = std::make_shared<Wolf>(x, y, id, heading);
   Patch& patch = this->get_patch(x, y); // Assumes a 'get_patch' function is defined
   std::weak_ptr<Turtle> new_wolf =wolf;
   patch.add_turtle(new_wolf);
   this->all_wolves.push_back(wolf);
}


void World::eat_grass_and_move(std::shared_ptr<Deer> deer) {
   Patch& currentPatch = *static_cast<Patch*>(deer->current_patch); // Get the current patch the deer is on

   double grassAvailable = currentPatch.grass; // Get the current grass amount

   double grass_eaten = std::min(grassAvailable, 30.0); // Consume up to 30 units of grass
   currentPatch.grass -= grass_eaten; // Update the grass amount on the current patch

   if (grass_eaten > 30.0) { // If more than 30 units of grass were consumed, look for a new patch with more grass
       Patch* new_patch = find_adjacent_patch_with_grass(deer, 30.0); // Find a new patch with more grass
       if (new_patch != nullptr) {
           deer->move_to(new_patch->coord); // Move the deer to the new patch
       } else { // If no suitable patch is found, the deer dies and grass on the current patch is set to 100
           currentPatch.grass = 100;
           // You may want to add code to remove the deer from the simulation or handle its death here
       }
   }
}

Patch* World::find_adjacent_patch_with_grass(Deer* deer_ptr, double minimum_grass) {
   int x = deer_ptr->getX();
   int y = deer_ptr->getY();
   for (int dx = -1; dx <= 1; ++dx) {
       for (int dy = -1; dy <= 1; ++dy) {
           ...
           auto patch_ptr = patches[nx][ny]; // Attempt to find a patch at the given coordinates
           if (patch_ptr == nullptr) {
               continue; // Skip empty patches
           }
           Patch& patch = *patch_ptr;
           double grass = patch.grass;
           if (grass >= minimum_grass) {
               return patch_ptr.get(); // Return the patch if it has enough grass
           }
       }
   }
   return nullptr; // Return nullptr if no patch with sufficient grass is found
}
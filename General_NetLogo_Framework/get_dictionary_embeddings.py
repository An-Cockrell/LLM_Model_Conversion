...
with open("dictionary_entries.json", "r") as f:
    data = json.load(f)
    # print(data)

    text_instruction_pairs = []
    for entry_num in data.keys():
        entry_name = data[entry_num]["Entry Name"]
        entry_data = data[entry_num]["Entry Data"]
        # print(entry_data)
        text_instruction_pairs.append({"instruction": "Represent the Documentation Dictionary Entry for retrieval:", "text": "Entry Name: " + entry_name + " \nEntry Data: " + entry_data})
    ...
    print("Time taken: " + str(time.time() - start))
    import numpy as np
    np.save("Netlogo_dictionary_embeddings", customized_embeddings)
